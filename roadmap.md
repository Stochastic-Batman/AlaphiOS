...
## Step 12: Monitor & Condition Variables

- `sync/spinlock.rs`: Upgraded `SpinlockGuard` field visibility to `pub(crate) lock: &'a Spinlock<T>` to allow cross-module raw pointer extraction without exposing unnecessary public getter methods.
- `sync/condvar.rs`: Implemented `CondVar` wrapping an internal `Spinlock<VecDeque<Pid>>`. The `wait` method enqueues the current task's PID, extracts the monitor's raw spinlock pointer, locks `TASK_TABLE` to update the task's state to `TaskState::Blocked` (adhering strictly to the `TASK_TABLE` $\rightarrow$ `SCHEDULER` $\rightarrow$ individual task lock hierarchy), drops the monitor guard to safely eliminate lost-signal race conditions, yields via `scheduler::schedule()`, and re-acquires the monitor lock upon waking. `signal()` and `broadcast()` pop from the queue and forward Pids to `scheduler::unblock(pid)`.
- `sync/monitor.rs`: Implemented `Monitor<T>` providing an RAII Mesa/Hoare-style concurrency pattern. Encapsulates a private `Spinlock<T>` and a private `CondVar`, exposing `lock()` and `try_lock()` wrappers alongside proxy methods for `wait()`, `signal()`, and `broadcast()`. This safely isolates underlying unsafe layout transitions and pointer-casting from high-level monitor state evaluation.


## Step 13: IPC with Pipes, Message Queues and Shared Memory

- `ipc/channel.rs`: Defined the uniform `IpcChannel` trait (`send`, `recv`, `close`) and the `IpcError` enum (`Closed`, `WouldBlock`, `TooLarge`) to abstract all inter-process communication mechanisms behind a common stream-oriented interface.
- `ipc/table.rs`: Implemented a centralized `IpcTable` tracking dynamic resources, mapping anonymous `u64` integer handles to abstract `Arc<dyn IpcChannel>` trait objects. Upgraded `IpcTable::new()` to a `pub const fn` to allow compile-time initialization inside the global static `IPC_TABLE: Spinlock<IpcTable>`.
- `ipc/pipe.rs` / `ipc/msg_queue.rs`: Created concrete channel implementations satisfying the `IpcChannel` boundary. Imported `core::cmp::min` to safely handle chunked buffer copies within bounded ring-buffer slices.
- `ipc/shm.rs`: Built the `SharedMem` struct for POSIX-like named shared memory. Implemented `IpcChannel` for it to explicitly return `Err(IpcError::WouldBlock)` on raw stream operations, allowing shared memory objects to be uniformly stored and dropped within the central `IPC_TABLE`.
- `syscall/dispatch.rs`: Established a global `SHM_REGISTRY` map for track-by-name lookups of shared regions. Expanded `syscall_handler` to dispatch `SYS_PIPE_CREATE`, `SYS_PIPE_READ`, `SYS_PIPE_WRITE`, `SYS_PIPE_CLOSE`, `SYS_MSG_SEND`, `SYS_MSG_RECV`, `SYS_SHM_OPEN`, and `SYS_SHM_CLOSE` through `IPC_TABLE` handles. Defined negative `isize` error constants matching standard ABI error definitions (`EBADF = -9`, `EAGAIN = -11`, `EFAULT = -14`, `EPIPE = -32`, `EMSGSIZE = -90`).
- `ipc/mod.rs` / `sync/mod.rs`: Registered all newly introduced submodules (`shm`, `pipe`, `msg_queue`, `table`, `condvar`, `monitor`) to fix core crate path resolution errors.
- `sync/monitor.rs`: Tied the monitor container reference lifetime directly to the lock guard scope (`pub fn wait<'a>(&'a self, ...)`) to satisfy borrow checker requirements during scheduled task yielding.
